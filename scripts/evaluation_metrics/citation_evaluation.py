...
from scripts.model_and_tokenizer import tokenizer, model, device  # Load pre-trained BERT model and tokenizer

# Load the dataset with citation references populated
# Ensure this CSV includes 'title', 'full_text', and 'citation_references' columns
...
def get_embedding(text):
    ...
    with torch.no_grad():
        ...
        # Use the mean of the hidden states for the embedding (alternatively, use CLS token)
...
# Define top-k for evaluation; determines the number of retrieved documents for each query
top_k = 5


# Function to calculate precision@k for a specific document (query_idx)
def precision_at_k(query_idx, top_k):
    # Get the embedding for the query document and compute similarity with all other embeddings
    query_embedding = embeddings[query_idx].reshape(1, -1)
    similarities = cosine_similarity(query_embedding, embeddings).flatten()

    # Set similarity with itself to -1 to exclude it from the top-k retrieved documents
    ...
    # Calculate precision@k by counting how many of the retrieved documents are in the true references
...
# Calculate the average Precision@k across all documents as the final evaluation metric
average_precision_at_k = np.mean(precision_scores)

# Print the result, showing the model's effectiveness at retrieving relevant (cited) documents
